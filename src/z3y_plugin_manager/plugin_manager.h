...
#include <filesystem>
#include <functional>
#include <map>              // [保留] 用于 std::weak_ptr 键 / loaded_libs_
#include <unordered_map>    // [!! 新增 !!] 用于高性能查找
#include <memory>
#include <mutex>
#include <queue>
#include <set>
...
// --- IEventBus 接口实现 ---
    ...
    private:
        ...
        // [!! 修改: 使用 unordered_map !!]
        std::unordered_map<ClassId, ComponentInfo> components_;  // [修改]
        // [!! 修改: 使用 unordered_map !!]
        std::unordered_map<ClassId, std::weak_ptr<IComponent>> singletons_;
        // [!! 恢复: 必须使用 std::map 来支持 rbegin()/rend() !!]
        ...
        // [!! 修改: 使用 unordered_map (via EventMap) !!]
        EventMap global_subscribers_;
        // [!! 修改: 使用 unordered_map (via SenderMap) !!]
        SenderMap sender_subscribers_;
        // [保留 map] global_sub_lookup_ 使用 map
